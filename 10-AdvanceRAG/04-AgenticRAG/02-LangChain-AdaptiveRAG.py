...
class GradeDocuments(BaseModel):
    ...
    binary_score: str = Field(description="Relevant: 'yes'|'no'.")
...
doc_grade_prompt = ChatPromptTemplate.from_messages([
    ("system", 
     "You are a relevance grader. A document is considered relevant if it contains keywords or semantics related to the question. Return 'yes' or 'no'."),
...
class GradeHallucinations(BaseModel):
    """Detect if the answer is fact-based: 'yes'|'no'."""
    binary_score: str = Field(description="Fact-based: 'yes'|'no'.")
...
hallu_prompt = ChatPromptTemplate.from_messages([
    ("system", "You are a hallucination detector. Determine if the answer is based on the provided facts. Return 'yes' or 'no'."),
...
class GradeAnswer(BaseModel):
    """Determine if the answer addresses the question: 'yes'|'no'."""
    binary_score: str = Field(description="Answer complete: 'yes'|'no'.")
...
ans_prompt = ChatPromptTemplate.from_messages([
    ("system", "You are an answer quality grader. Determine if the answer solves the question. Return 'yes' or 'no'."),
    ("human", "User question:\n\n{question}\n\nLLM generation: {generation}")
])
...
re_write_prompt = ChatPromptTemplate.from_messages([
    ("system", 
     "Rewrite the input question into a version more suitable for vector retrieval, maintaining semantic intent."),
...
# Return to retrieval after document rewriting
wf.add_edge("transform_query", "retrieve")
# Web search directly to generation
...
try:
    ...
    png_data = app.get_graph(xray=True).draw_mermaid_png(
        draw_method=MermaidDrawMethod.PYPPETEER  # Use local browser rendering, no external service needed
    )
