...
# Custom retrieval node, merge historical messages
def retrieve(state: AgentState) -> AgentState:
    msgs = state['messages']
    tool = tools[0]  # There is only one retrieval tool here
    question = msgs[0].content
...
def agent(state: AgentState) -> AgentState:
    ...
    if not msgs:
        raise ValueError("Message list is empty when calling agent node, unable to generate response. Please check upstream node output.")
    
    # Add system message to guide the use of retrieval tools
...
def should_use_tools(state: AgentState) -> str:
    msgs = state['messages']
    last_msg = msgs[-1]
    # Check if message content contains tool calls or needs retrieval
...
def rewrite(state: AgentState) -> AgentState:
    msgs = state['messages']
    question = msgs[0].content
    prompt = HumanMessage(content=f"Rewrite the following question for better document retrieval:\n{question}\n")
    model = ChatOpenAI(temperature=0, model="gpt-4o", streaming=True)
...
# ----- 6. Run Example -----
# Initialize input messages, using HumanMessage type
...
# Run and print output of each node
final_output = None
for output in app.stream(inputs):
    print("\n=== Node Output ===")
    # Print name and state of each node
    for node_name, state in output.items():
        ...
        print(f"Grading Result: {state.get('grade_result')}")
    print("===============")
    final_output = output
...
if final_output:
    ...
    print("\n=== Error: Failed to get final output ===")