...
# 2. Initialize the OpenAI API (using the latest Response API)
openai.api_key = os.getenv("OPENAI_API_KEY")

# It's recommended to use the new Response API style
...
def retrieve(collection: str, query_emb: list, top_k: int = 3, fields: list = None):
    ...
    logging.info(f"[Retrieval] {collection} retrieval results: {results}")
    return results[0]  # Return the result list for the first query

# 8. SQL extraction function
def extract_sql(text: str) -> str:
    # Try to match a SQL code block
    ...
    # If neither is found, return the raw text
    return text.strip()

# 9. Execute the SQL and return the results
...
def generate_sql(prompt: str, error_msg: str = None) -> str:
    if error_msg:
        prompt += f"\nThe previous SQL execution failed with this error: {error_msg}\nPlease fix the SQL statement:"
...
# 11. Core flow: natural language -> SQL -> execute -> return
def text2sql(question: str, max_retries: int = 3):
    # 11.1 Embed the user's question
    ...
    # 11.4 RAG retrieval: column descriptions
    desc_hits = retrieve("dbdesc_knowledge", q_emb.tolist(), top_k=5, fields=["table_name", "column_name", "description"])
    logging.info(f"[Retrieval] Column description retrieval results: {desc_hits}")
    try:
        desc_context = "\n".join(
        ...
        logging.error(f"[Retrieval] Column description processing error: {e}")
        desc_context = ""

    # 11.5 Assemble the base prompt
    base_prompt = (
    ...
    # 11.6 Generate and execute the SQL, retrying up to max_retries times
    error_msg = None
    ...
    print(f"Execution failed after reaching the maximum of {max_retries} retries.")
    print("Last error message:", error_msg)
