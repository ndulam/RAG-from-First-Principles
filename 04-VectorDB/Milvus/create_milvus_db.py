...
# Get the vector dimension (using a sample document)
...
fields = [
    ...
    FieldSchema(name="vector", dtype=DataType.FLOAT_VECTOR, dim=vector_dim), # BGE-m3 is the most important
...
# # Add an index after creating the collection
index_params = client.prepare_index_params()
index_params.add_index(
    field_name="vector",  # specify which field to create the index for, here the vector field
    index_type="AUTOINDEX",  # use the automatic index type; Milvus will pick the best index based on the data characteristics
    metric_type="COSINE",  # use cosine similarity as the vector similarity metric
    params={"nlist": 1024}  # index parameter: nlist is the number of cluster centers; a larger value gives higher search accuracy but lower speed
)
...
# Search for the highest cosine similarity matches
