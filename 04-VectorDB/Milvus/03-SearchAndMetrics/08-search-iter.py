...
iterator = client.search_iterator(
    ...
    limit=20000,      # Return 20000 results in total
    output_fields=["color"]
)

# Use the iterator to fetch results
all_results = []
while True:
    ...
    # Convert each result to a dict and add it to the results list
    for hit in result:
        all_results.append(hit.to_dict())

print(f"Retrieved {len(all_results)} results in total")
print("\nFirst 5 results:")
